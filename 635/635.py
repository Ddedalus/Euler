...
def binom_pre_computed(facts, invfacts, n, k, mod):
    # n! / (k!^(mod-2) * (n-k)!^(mod-2)) (mod p)
    return (facts[n] * ((invfacts[k]*invfacts[n-k] % mod))) % mod
...
def A_pre_computed(facts, primes, n, q, mod):
    ...
    #                                     * ( inv(n!)^2 * inv(((q-1)n)!) )
    if(q == 2):
        num = (facts[n*q] + modprod(mod, (n-1)*q, facts[n], facts[(q-1)*n]))* facts[n-1] % mod
...
if __name__ == '__main__':
    ...
    n = 11
    print(S_pre_computed(facts, primes, L, 3, mod))
